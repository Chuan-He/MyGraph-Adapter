...
class GraphNN(nn.Module):
    def __init__(self, nfeat):
        ...
        nf = 512
        self.module_w0 = Wcompute(nfeat, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
        self.module_l0 = Gconv(nfeat, int(nf / 2), 2)
        self.module_w1 = Wcompute(nfeat + int(nf / 2), nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
        self.module_l1 = Gconv(nfeat + int(nf / 2), int(nf / 2), 2)
        self.module_w2 = Wcompute(nfeat + nf, nf, operator='J2', activation='softmax', ratio=[2, 2, 1, 1])
