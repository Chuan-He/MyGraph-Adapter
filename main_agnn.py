...
def get_arguments():
    ...
    parser.add_argument('--test_N_shots', type=int, default=16, metavar='test_N_shots',
                        help='Number of shots in test')
    parser.add_argument('--train_N_shots', type=int, default=16, metavar='train_N_shots',
...
#   cp_exist = utils.ensure_path(save_path)
    ...
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=n_train_shot, tfm=train_tranform, is_train=True, shuffle=False)
    train_loader_sample = build_data_loader1(data_source=dataset.train_x, batch_sampler=train_sampler, tfm=train_tranform, is_train=True)
    ...
    test_loader = build_data_loader1(data_source=dataset.test, batch_sampler=test_sampler, tfm=train_tranform, is_train=False)
    ...
    gnn_model = GraphNN(nfeat=1024).to('cuda')
    gnn_model_text = GraphNN(nfeat=1024).to('cuda')
    device_count = torch.cuda.device_count()
    if device_count > 1:
        print(f"Multiple GPUs detected (n_gpus={device_count}), use all of them!")
        gnn_model = nn.DataParallel(gnn_model)
        gnn_model_text = nn.DataParallel(gnn_model_text)
    # NOTE: only give prompt_learner to the optimizer
    optimizer = torch.optim.AdamW(list(gnn_model.parameters())+list(gnn_model_text.parameters()), lr=cfg['lr'], eps=1e-4)
