...
class DatasetBase:
    ...
    def get_task_batch(self, data_source, batch_size=5, n_way=20, num_shots=1, unlabeled_extra=0, tfm=None):
        DataDict = self.split_dataset_by_label(data_source)
        dataset_wrapper = DatasetWrapper(data_source, input_size=224, transform=tfm, is_train=True)
        # Init variables
        batch_x = np.zeros((batch_size, 3, 224, 224), dtype='float32')
        ...
        for i in range(n_way*num_shots):
            batches_xi.append(np.zeros((batch_size, 3, 224, 224), dtype='float32'))
        ...
        for batch_counter in range(batch_size):
            ...
            for class_counter, class_ in enumerate(sampled_classes):
                if class_counter == positive_class:
                    ...
                    samples = random.sample(DataDict[class_], num_shots+1)
                    # Test sample is loaded
                    batch_x[batch_counter, :, :, :] = dataset_wrapper.getitem(samples[0])
                    labels_x[batch_counter, class_counter] = 1
                    samples = samples[1::]
                else:
                    samples = random.sample(DataDict[class_], num_shots)

                for s_i in range(0, len(samples)):
                    batches_xi[indexes_perm[counter]][batch_counter, :, :, :] = dataset_wrapper.getitem(samples[s_i])
        ...
        return_arr = [torch.from_numpy(batch_x), torch.from_numpy(labels_x), batches_xi, labels_yi, oracles_yi, torch.from_numpy(hidden_labels)]
