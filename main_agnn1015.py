...
#   cp_exist = utils.ensure_path(save_path)
    ...
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=256, tfm=train_tranform, is_train=True, shuffle=False)
    ...
    gnn_model = GraphNN(nfeat=1024, nhid=512, nclass=n_classes).to('cuda')
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, label_global in tqdm(train_loader_sample, desc='train', leave=False): # 4, 5, 6   120
            # print(data.size())
            ...
            t_gcn1 = gnn_model(tt.unsqueeze(0)).squeeze(0)
            ...
            for ep in range(ep_per_batch):
                logits1 = clip_model.logit_scale * xgcn1_sup[ep] @ tgcn1.t() #clip_model.logit_scale
                # logits2 = clip_model.logit_scale * xgcn2_sup[ep] @ tgcn2[label_global_idx].t()
                logits_sup.append(logits1)

                logits_1 = clip_model.logit_scale * xgcn1_query[ep] @ tgcn1.t()
                # logits_2 = clip_model.logit_scale * xgcn2_query[ep] @ tgcn2[label_global_idx].t()
                logits_query.append(logits_1)
